#!/usr/bin/env python

######################################################################
# Pack extracted histos into rootfiles
# Works with v400 and later versions
# 
# version: 2020-04-29
# 
######################################################################

import os,sys
import numpy as np

import ROOT
from ROOT import *
ROOT.gROOT.Reset()
ROOT.gErrorIgnoreLevel = kWarning

sys.path.append("/home/mkauer/COSINE/CUP/mc-fitting/")
sys.path.append("/home/mkauer/mc-fitting/")

### REMEMBER to change the version here if it changed!
#from funcs400 import *
# Why did I use funcs400 here? Maybe this was before
#   I had a funcs410?
# Switching to funcs410 for testing
from funcs410 import *

local = amLocal()
gROOT.SetBatch(1)

def _myself_(argv):

    ### REMEMBER to change the version here for the new directory!
    build = 'build500'
    
    xstal = 0
    
    if len(sys.argv)>1:
        mcfile = str(sys.argv[1])
    else:
        print "ERROR: specify a backgrounds or data file"
        return
    
    #if len(sys.argv)>2:
    #    xstal = int(sys.argv[2])
    
    ### select a crystal or comment out
    #xstal = 7
    
    if local: outpath = "/home/mkauer/COSINE/CUP/mc-fitting/root-join-read/"+build+'/'
    else:     outpath = "/home/mkauer/mc-fitting/root-join-read/"+build+'/'
    if xstal: outpath += 'c'+str(xstal)+'/'
    if not os.path.exists(outpath):
        try:    os.path.mkdir(outpath)
        except: os.mkdir(outpath)
    fname = mcfile.split('/')[-1]
    rootfile = build+'-'+fname+'.root'
    rfile = TFile(outpath+rootfile, 'RECREATE')
    
    ### REMEMBER to change the version here if it changed!
    if xstal: data, bkgs, sigs = build410(mcfile, 1, 2, 'SM', [xstal])
    else:     data, bkgs, sigs = build410(mcfile, 1, 2, 'SM')

    rfile.Write()
    rfile.Close()
    
    print "\nDONE\n"
    
    
if __name__ == "__main__":
    _myself_(sys.argv[1:])

